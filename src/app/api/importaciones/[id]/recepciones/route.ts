  ...
  try {
    ...
    const { data: rec, error } = await ctx.supabase
      ...
      .insert({
        empresa_id: emp,
        importacion_id: id,
        fecha: typeof b.fecha === "string" && b.fecha ? b.fecha.slice(0, 10) : new Date().toISOString().slice(0, 10),
        ubicacion_id: b.ubicacion_id ? String(b.ubicacion_id) : null,
        final,
      ...
      .single();
    if (error) throw new Error(error.message);
    const recepcionId = (rec as { id: string }).id;
    if (lineas.length) {
      ...
      if (ins.error) {
        await ctx.supabase.from("importacion_recepciones").delete().eq("id", recepcionId);
        throw new Error(ins.error.message);
      }
    }

    // Recibido = suma de todas las recepciones (no se acumula a mano: no se desfasa).
    ...
    for (const r of (todas ?? []) as unknown as { importacion_recepcion_items: { item_id: string; cantidad: number }[] }[])
      for (const l of r.importacion_recepcion_items ?? []) recibido.set(l.item_id, (recibido.get(l.item_id) ?? 0) + Number(l.cantidad));
    await Promise.all(
      items.map((i) =>
        ctx.supabase
          .from("importacion_items")
          .update({ cantidad_recibida: recibido.get(i.id) ?? 0, updated_at: new Date().toISOString() })
          .eq("empresa_id", emp)
          .eq("id", i.id)
      )
    );
    ...
    if (diferencias.length) {
      const { data: inc } = await ctx.supabase
        ...
        .select("id")
        .single();
      incidenciaId = (inc as { id: string } | null)?.id ?? null;
    }
