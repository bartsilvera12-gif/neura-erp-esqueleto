...
export async function POST(request: NextRequest, p: { params: Promise<{ id: string }> }) {
  try {
    ...
    const sinProducto = lineas.filter((l) => !l.producto_id).map((l) => l.descripcion);
    ...
    const { data: prods } = await ctx.supabase.from("productos").select("id, nombre, sku").eq("empresa_id", emp).in("id", conProducto.map((l) => l.producto_id as string));
