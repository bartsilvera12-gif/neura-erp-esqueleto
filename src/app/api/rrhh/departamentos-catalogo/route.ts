...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json(successResponse({ departamentos: data ?? [] }));
  } catch (err) {
    console.error("[/api/rrhh/departamentos-catalogo GET]", err);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: exist } = await ctx.supabase
      .from("departamentos_catalogo").select("slug").eq("empresa_id", ctx.auth.empresa_id);
    ...
    const { data, error } = await ctx.supabase.from("departamentos_catalogo").insert({
      empresa_id: ctx.auth.empresa_id, slug, nombre, activo: true, orden, es_sistema: false,
    }).select().single();
    if (error) return NextResponse.json(errorResponse(error.message), { status: 400 });
    return NextResponse.json(successResponse({ departamento: data }), { status: 201 });
  } catch (err) {
    console.error("[/api/rrhh/departamentos-catalogo POST]", err);
    return NextResponse.json(errorResponse("Error"), { status: 500 });
  }
}
