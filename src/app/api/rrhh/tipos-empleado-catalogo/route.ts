...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error("[/api/rrhh/tipos-empleado-catalogo GET]", err);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: exist } = await ctx.supabase
      .from("tipos_empleado_catalogo").select("slug").eq("empresa_id", ctx.auth.empresa_id);
    ...
    const { data, error } = await ctx.supabase.from("tipos_empleado_catalogo").insert({
    ...
    console.error("[/api/rrhh/tipos-empleado-catalogo POST]", err);
    return NextResponse.json(errorResponse("Error"), { status: 500 });
  }
}
