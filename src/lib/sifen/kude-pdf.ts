...
/**
 * Logo por defecto del KuDE. Primero el de la instancia (Esqueleto ERP); si no está,
 * se preserva el logo Neura del bundle.
 */
function readLogoBytes(): Uint8Array | null {
  const candidatos = [
    path.join(process.cwd(), "public", "brand", "esqueleto-logo.png"),
...
export async function buildKudePdfBuffer(input: BuildKudePdfInput): Promise<Buffer> {
  ...
  pdfDoc.setAuthor("Esqueleto ERP");
  ...
  leg += 2;
  page.drawText("Generado con Esqueleto ERP", {
    x: margin + footPad,
    y: baselineFromTop(page, leg),
    size: 6.5,
    font,
